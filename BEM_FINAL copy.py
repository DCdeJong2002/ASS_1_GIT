...
# ── Display ───────────────────────────────────────────────────────────────────
SHOW_PLOTS = True   # False -> save and close immediately (non-interactive)
...
def _make_objective(quartic):
    def obj(params):
        ...
        if c_min < CHORD_MIN:      pen += 10000*(CHORD_MIN - c_min)**2
        if c_max > CHORD_MAX_REG:  pen += 1000*(c_max - CHORD_MAX_REG)**2
        if c_min < CHORD_MIN*0.5:  return 1e9
        r, c, tw = build_poly_geometry(params)
        CT, CP, _ = evaluate_rotor(r, c, tw)
        pen += 3000*(CT - CT_TARGET)**2
        if quartic:
            _, _, _, t_curve, _, c2, c3, c4 = params
            pen += 0.005*t_curve**2 + 0.001*(c2**2 + c3**2 + c4**2)
        else:
            _, _, _, t_curve, _, c2, c3 = params
            pen += 0.005*t_curve**2 + 0.001*(c2**2 + c3**2)
        return -CP + pen
    return obj
...
def run_poly_optimizer(quartic=False,n_starts=N_STARTS,seed=42):
    ...
    if quartic:
        bounds=[(-4,4),(-25,5),(-10,15),(-20,20),(0.3,2),(-3,3),(-3,3),(-3,3)]
        ...
        bounds=[(-4,4),(-25,5),(-10,15),(-20,20),(0.3,2),(-3,3),(-3,3)]
