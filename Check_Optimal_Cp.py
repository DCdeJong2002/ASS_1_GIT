...
def OptimalChordFunction(r_R):
    return 3.2458 * (1 - r_R) + 0.8827

def OptimalTwistFunction(r_R, pitch):
    return -(20.0683 * (1 - r_R) + pitch)

Pitch = -6.0805   # Set blade pitch angle (degrees)
...
for TSR in [6, 8, 10]:
    ...
    for i in range(len(r_R_bins) - 1):
        ...
        chord_function = OptimalChordFunction(r_mid)
        twist_function = OptimalTwistFunction(r_mid, Pitch)
