...
def RunFullSimulation(c2, c1, c0, t2, t1, pitch, TSR_val):
    Omega = U0 * TSR_val / Radius
    temp_res = []
    for i in range(len(r_R_bins) - 1):
        rm = (r_R_bins[i] + r_R_bins[i+1]) / 2
        chord = c2 * rm**2 + c1 * rm + c0
        twist = -(t2 * rm**2 + t1 * rm + pitch)
...
## ================================================================== ##
## Design Optimization & Precise Scaling (Quadratic Polynomial)
## ================================================================== ##
...
# Fit quadratic curves directly to r/R
# polyfit with degree 2 returns 3 coefficients: ax^2 + bx + c
c2, c1, c0 = np.polyfit(r_range, c_req, 2)

# Since twist = -(t2*rm**2 + t1*rm + pitch), we fit the negative of t_req
t2, t1, pitch = np.polyfit(r_range, -np.array(t_req), 2)

# 2. Scaling Loop: Adjust geometry until "Correct CT" hits 0.7500
print("Scaling quadratic chord to reach exact CT target...")
for i in range(15):
    CT_current, _, _ = RunFullSimulation(c2, c1, c0, t2, t1, pitch, 8.0)
    if abs(CT_current - 0.75) < 0.00005: break
    # Scale all chord coefficients by the deficit
...
for tsr_choice in [6, 8, 10]:
    ct_val, cp_val, res_data = RunFullSimulation(c2, c1, c0, t2, t1, pitch, tsr_choice)
...
print("-" * 40)
print(f"Final Twist: -({t2:.4f} * r/R^2 + {t1:.4f} * r/R + {pitch:.4f}) deg")
print(f"Final Chord: ({c2:.4f} * r/R^2 + {c1:.4f} * r/R + {c0:.4f}) m")
...
# Convert the old linear baseline into the new quadratic inputs (where x^2 = 0)
c2_old, c1_old, c0_old = 0.0, -3.0, 4.0
t2_old, t1_old, pitch_old_conv = 0.0, -14.0, 12.0

ct_old, cp_old, results_old = RunFullSimulation(c2_old, c1_old, c0_old, t2_old, t1_old, pitch_old_conv, 8.0)

# 2. Create Plotting Dashboard
fig, axs = plt.subplots(2, 2, figsize=(15, 12))
fig.suptitle(f'Comparison: Baseline vs. Optimized Blade (TSR=8)\n'
             f'Old: CP={cp_old:.4f}, CT={ct_old:.4f} | New: CP={cp_val_tsr8:.4f}, CT={ct_val_tsr8:.4f}', fontsize=16)
...
# Chord
ax1.plot(r_R, c2_old*r_R**2 + c1_old*r_R + c0_old, 'k--', label='Old Chord [m]')
ax1.plot(r_R, c2*r_R**2 + c1*r_R + c0, 'b-', label='New Quadratic Chord [m]')
ax1.set_ylabel('Chord [m]', color='b')
ax1.tick_params(axis='y', labelcolor='b')

# Twist on twin axis (plotting the inner part of the twist function to match previous graphs)
ax1b = ax1.twinx()
ax1b.plot(r_R, t2_old*r_R**2 + t1_old*r_R + pitch_old_conv, 'k:', label='Old Twist [deg]')
ax1b.plot(r_R, t2*r_R**2 + t1*r_R + pitch, 'r-', label='New Quadratic Twist [deg]')
ax1b.set_ylabel('Twist [deg]', color='r')
ax1b.tick_params(axis='y', labelcolor='r')

ax1.grid(True)
ax1.legend(loc='upper right', fontsize='small')
ax1b.legend(loc='lower left', fontsize='small')
